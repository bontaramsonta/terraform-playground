exports.handler = async (event, context) => {
    ...
    const output = event.records.map((record) => {
        ...
        try {
            ...
            const mobileEvSourceArray = get(["endpoint", "Attributes", "app_type"], jsonEvent);
            const mobileEVSource = mobileEvSourceArray && mobileEvSourceArray.length ? mobileEvSourceArray[0] : null;
            const evSource = (mobileEVSource ? JSON.stringify(mobileEVSource) : JSON.stringify(get(["attributes", "app_type"], jsonEvent))).replace(/\\"/g, '""');
            ...
            const mobileuIDArray = get(["endpoint", "Attributes", "uID"], jsonEvent);
            const androidUIDArray = get(["attributes", "uID"], jsonEvent).replace(/\\"/g, '""');
            const uId = uIdArray && uIdArray.length ? uIdArray[0] : null;
            const mobileuId = mobileuIDArray && mobileuIDArray.length ? mobileuIDArray[0] : null;
            const androidUID = androidUIDArray && androidUIDArray.length ? androidUIDArray : null;
            const userId = JSON.stringify(uId || mobileuId || androidUID || '0');
